...
def camac_init(sock):
    ...
    print "r", 
    print camac_control.rsync(sock)
    print "c", 
    print camac_control.cccc(sock)
    print "z", 
    print camac_control.cccz(sock)
    print "i", 
    print camac_control.ccci(sock)
...
def read_mod(sock, n, sub_addr_max, grp1=True):
    ...
    for addr in range(sub_addr_max):
        ...
        res.append(camac_control.cssa(sock, n, func, addr)[1])
    return res
...
def reset_trigger(sock, n, subaddr, grp1=True, addr=0):
    ...
    status = camac_control.cssa(n, func, a=addr, data=subaddr)[3]
    if status != 128: raise Exception ("no qresp")
    # now clear the address
    status = camac_control.cssa(sock, n, func, a=addr, data=0)[3]
    if status != 128: raise Exception("no qresp")
